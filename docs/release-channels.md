...
The identity service publishes to two channels. They mean exactly what the desktop
...
client against a beta server and "beta" has to be one idea, not two. This
file is deliberately the same document as the chat server's
`docs/release-channels.md`; the two repos release in lockstep.
...
Published to `ghcr.io/bsfchat/identity`.
...
2. builds the image **once**, with `-DBSFCHAT_ID_VERSION` set from the
...
The version is a `-DBSFCHAT_ID_VERSION` CMake define
...
docker logs bsfchat-identity | head -1
#   BSFChat identity 0.0.44 (rev 1a2b3c4, stable channel)

# 2. the OIDC discovery document (additive, vendor-namespaced; OpenID
#    Connect Discovery 1.0 section 3 requires clients to ignore metadata
#    they do not recognise, so no relying party is affected)
curl -s http://localhost:8480/.well-known/openid-configuration | jq
#   { "issuer": "...", ...,
#     "bsfchat_version": "0.0.44",
#     "bsfchat_revision": "1a2b3c4",
#     "bsfchat_channel": "stable" }

# 3. OCI labels, without starting anything
docker inspect -f '{{index .Config.Labels "org.opencontainers.image.version"}}' \
  ghcr.io/bsfchat/identity:latest
```
