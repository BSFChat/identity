...
// included by main.cpp, by OidcHandler (for the discovery document) and
// by the unit tests, and none of those should acquire a dependency on the
// others through this.
//
// BSFCHAT_ID_VERSION / BSFCHAT_ID_REVISION are compile
...
#ifndef BSFCHAT_ID_VERSION
#define BSFCHAT_ID_VERSION "0.0.0-dev"
#endif

#ifndef BSFCHAT_ID_REVISION
#define BSFCHAT_ID_REVISION "unknown"
#endif

namespace bsfchat::id::build {

inline constexpr std::string_view kVersion = BSFCHAT_ID_VERSION;
inline constexpr std::string_view kRevision = BSFCHAT_ID_REVISION;
...
//
// Mirrors client/src/core/ReleaseSelection.h's parseVersion (and the chat
// server's src/core/Version.h) so every part of a deployment agrees on
...
// for the startup banner and the discovery document. An unparseable
